                ...
                with open(sys.argv[1]+'mystat') as f_stat:
                    for line_stat in f_stat:
                        ...
                        if indiv_name == indiv:
                            ...
                            acc_trg += (float) (line_stat.split(' ')[4])
                            break
        else:
            if skip_gen == 0:
                newgen = 1;
...
first = True
last_gen = -1
print "gen avg_1 best_1 worst_1 avg_2 best_2 worst_2 avg_trigger"
    ...
    for line in f:
        ...
        if gen != last_gen:
            ...
            if (not first) and int(gen) < len(trg_list):    
                print gen+' '+str(param1[0])+' '+' '+str(param1[1])+' '+str(param1[2])+' '+str(param2[0])+' '+' '+str(param2[1])+' '+str(param2[2])+' '+str(trg_list[int(gen)][0])
            first = False
#        k = 0
#        for i in line: 
#            print str(k)+' '+i
