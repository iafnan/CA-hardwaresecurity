...
i=$2
OR_DIR=../or1200_$i
SOURCEFILE=sources/fast9_detect_O2.S
SOURCEFILE_BYTES=sources/fast9_detect_bytes
...
cp tmp$i.S $OR_DIR/sw/selftest/fast9_detect_O2.S
...
if [[ $(grep "Error" log$i.log | wc -c) -eq 0 ]]; then
	...
	awk '/<fast9_detect>:/ {p=1}; p; /^$/ {p=0}' $OR_DIR/sw/selftest/selftest-nocache.lst | tail -n +2 | head -n -1 | cut -f2 | sed 's/ //g' | tr '\n' ' ' > byte$i
